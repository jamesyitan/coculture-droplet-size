...
ft = fittype( 'K./(1+((K-0.1)/0.1)*exp(-r*(t-x)))', 'independent', 't', 'dependent', 'y' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [0.950222048838355 0.0344460805029088 10];

% Fit model to data.
[fitresult1, gof1] = fit( xData1, yData1, ft, opts );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [150 1 200];
[fitresult2, gof2] = fit( xData2, yData2, ft, opts );

%%change opt start point for third fit
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [400 1 50];
